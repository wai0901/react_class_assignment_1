...
import { Breadcrumb, BreadcrumbItem ,
    Button, Form, FormGroup, Label, Input, Col, FormFeedback, Fade } from "reactstrap";
    ...
    validate(firstName, lastName, phoneNum, email) {
        ...
        if (this.state.touched.firstName) {
            if (firstName.length < 2) {
                ...
                error.firstName = "First name must be 15 or less characters";
            }
        }
        if (this.state.touched.lastName) {
            if (lastName.length < 2) {
                error.lastName = "First name must be at least 2 characters";
            } else if (lastName.length > 15) {
                error.lastName = "First name must be 15 or less characters";
            }
        }

        const reg = /^\d+$/;
        if (this.state.touched.phoneNum && !reg.test(phoneNum)) {
            error.phoneNum = "The phone number should contain only numbers";
        }
